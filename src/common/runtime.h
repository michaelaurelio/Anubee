...
//
// BPF-dependent helpers (anubee_libbpf_quiet, anubee_drops_read) are declared as
...
#ifndef __ANUBEE_COMMON_RUNTIME_H
#define __ANUBEE_COMMON_RUNTIME_H
...
// ANUBEE_DEBUG-gated libbpf print callback. Pass to libbpf_set_print().
...
static inline int anubee_libbpf_quiet(enum libbpf_print_level lvl,
                                     const char *fmt, va_list ap)
{
    if (lvl == LIBBPF_DEBUG && !getenv("ANUBEE_DEBUG"))
        return 0;
...
static inline int anubee_rb_poll_until_cb(struct ring_buffer *rb,
...
static inline void anubee_rb_poll_until(struct ring_buffer *rb,
                                      volatile sig_atomic_t *stop)
{
	(void)anubee_rb_poll_until_cb(rb, stop, NULL, NULL);
}
...
static inline void anubee_rb_poll_multi(struct ring_buffer **rbs, int n,
                                      volatile sig_atomic_t *stop)
{
	while (!*stop) {
		int total = 0;
...
static inline unsigned long long anubee_drops_read(int map_fd)
{
...
#endif /* __ANUBEE_COMMON_RUNTIME_H */
