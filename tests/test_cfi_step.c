...
int main(void)
{
	...
	int r = cfi_step(&s, 0x140, x, &sp, &pc, stack, 0x8000, sizeof(stack));
	assert(r == 1);
	...
	assert(cfi_step(&s, 0x140, x2, &sp2, &pc2, stack, 0x8000, sizeof(stack)) == 0);

	/* pc with no FDE -> stop */
	uint64_t x3[31]; memset(x3,0,sizeof(x3));
	uint64_t sp3=0, pc3=0x9999;
	assert(cfi_step(&s, 0x9999, x3, &sp3, &pc3, stack, 0x8000, sizeof(stack)) == 0);
