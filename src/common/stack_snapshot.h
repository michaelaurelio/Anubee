 ...
 * before this header).
 */
#ifndef ANUBEE_STACK_SNAPSHOT_H
#define ANUBEE_STACK_SNAPSHOT_H
...
#define ANUBEE_SNAP_MAX   32768
#define ANUBEE_SNAP_MID   8192
#define ANUBEE_SNAP_SMALL 2048
#define ANUBEE_SNAP_NREG  31   /* x0..x30 */

/* Per-chunk granularity for fault-tolerant capture (W3-window): the stack is
 * read in ANUBEE_SNAP_CHUNK-sized pieces, stopping at the first unmapped page, so
...
#define ANUBEE_SNAP_CHUNK 4096
_Static_assert(ANUBEE_SNAP_MAX % ANUBEE_SNAP_CHUNK == 0,
               "ANUBEE_SNAP_MAX must be a whole number of chunks (loop drops the remainder)");
...
struct anubee_stack_snapshot {
	...
	__u64 regs[ANUBEE_SNAP_NREG];      /* full GP file x0..x30 (CFI initial state) */
	...
	__u8  truncated;                  /* 1 = snap_len==ANUBEE_SNAP_MAX (window-capped, stack may extend beyond); 0 = a chunk faulted = reached stack_base = all mapped stack captured */
	__u8  _pad[3];
	__u8  snap[ANUBEE_SNAP_MAX];        /* user stack bytes starting at sp */
};
...
void anubee_stack_snapshot_emit_json(struct jbuf *j,
...
struct anubee_unwind_regs {
	__u64 x[ANUBEE_SNAP_NREG];  /* x0..x30 */
	__u64 sp;
	__u64 pc;
};

static inline void unwind_regs_from_snapshot(const struct anubee_stack_snapshot *s,
					     struct anubee_unwind_regs *out)
{
