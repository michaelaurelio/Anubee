...
// anubee_analyzer_t — interface every `anubee mod <name>` analyzer implements.
...
#ifndef __ANUBEE_ANALYZER_H
#define __ANUBEE_ANALYZER_H

struct anubee_sink;   // common/emit.h — full def needed by callers that open a sink
...
struct anubee_mod_ctx {
    struct anubee_sink *sink;  // NULL when no -o (console-only mode)
    ...
    const char *pkg;          // launched package (-P), or best-effort resolved
                              // in -p mode via anubee_resolve_pkg_from_pid; NULL
...
typedef struct {
    ...
    const char *description; // shown in `anubee mod --help`
    ...
    void (*emit_summary)(struct anubee_sink *sink);
...
extern const anubee_analyzer_t analyzer_proc_event;
extern const anubee_analyzer_t analyzer_execve;
extern const anubee_analyzer_t analyzer_prop_read;
extern const anubee_analyzer_t analyzer_file_access;
extern const anubee_analyzer_t analyzer_massdelete_detect;
extern const anubee_analyzer_t analyzer_exfil_detect;
extern const anubee_analyzer_t analyzer_accessibility_detect;
extern const anubee_analyzer_t analyzer_fileless_detect;
extern const anubee_analyzer_t analyzer_screencapture_detect;

#endif /* __ANUBEE_ANALYZER_H */
