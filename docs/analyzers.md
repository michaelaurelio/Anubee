...
ares mod ransomware-burst -P com.example.app -o out.jsonl
...
| `ransomware-burst` | Rapid rename+delete bursts on external storage (20 touches / 10s, mostly-distinct files) | Stealthy |
...
## Testing `ransomware-burst`/`exfil-burst` against a real app (manual)

The burst analyzers key off genuine app-UID file activity. `scripts/burstapp/build.sh install`
...
- **`ransomware-burst`/`exfil-burst` need `MANAGE_EXTERNAL_STORAGE`** ("All files
  access") on the target app. Scoped storage (Android 11+) otherwise blocks the
  signal outright. `ransomware-burst` surfaces whether the app holds it.
- **Deletes via MediaStore's trash API are invisible** to `ransomware-burst`.
