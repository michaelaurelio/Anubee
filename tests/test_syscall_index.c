...
static const char *ref_name(const struct anubee_sysent *t, size_t n, long nr) {
...
int main(void) {
    ...
    static const struct anubee_sysent tbl[] = {
        { 0,   "read"  },
        { 1,   "write" },
        { 56,  "openat"},
        { 63,  "close" },
        { ANUBEE_SYS_NR_CAP + 7, "high_cold" },
    };
    const size_t n = sizeof(tbl) / sizeof(tbl[0]);

    struct anubee_sysindex ix;
    anubee_sysindex_build(&ix, tbl, n);

    // Every table entry resolves, and matches the reference scan.
    for (size_t i = 0; i < n; i++) {
        const char *got = anubee_sysindex_name(&ix, tbl[i].nr);
    ...
    // Hot-path spot check.
    check(strcmp(anubee_sysindex_name(&ix, 56), "openat") == 0, "hot-path openat");
    // Cold-path (nr >= cap) still resolves via the retained linear fallback.
    check(strcmp(anubee_sysindex_name(&ix, ANUBEE_SYS_NR_CAP + 7), "high_cold") == 0,
          "cold-path nr >= cap");
    // Unknown nrs return NULL (both below cap and negative).
    check(anubee_sysindex_name(&ix, 999) == NULL, "unknown low nr -> NULL");
    check(anubee_sysindex_name(&ix, -1) == NULL, "negative nr -> NULL");

    // Idempotency: rebuilding yields identical lookups.
    anubee_sysindex_build(&ix, tbl, n);
    check(strcmp(anubee_sysindex_name(&ix, 1), "write") == 0, "idempotent rebuild");
