...
MOD_CSRC   := $(SRC)/modules/mod_emit.c $(SRC)/modules/file_access_classify.c $(SRC)/modules/massdelete_detect_classify.c $(SRC)/modules/accessibility_detect_classify.c $(SRC)/modules/mediaproj_abuse_parse.c $(SRC)/modules/proc_event.c $(SRC)/modules/execve.c $(SRC)/modules/prop_read.c $(SRC)/modules/file_access.c $(SRC)/modules/massdelete_detect.c $(SRC)/modules/exfil_detect.c $(SRC)/modules/accessibility_detect.c $(SRC)/modules/fileless_detect.c $(SRC)/modules/mediaproj_abuse.c $(SRC)/modules/mod.c
...
$(BUILD)/modules/%.o: $(SRC)/modules/%.c $(PROC_EVENT_SKEL) $(EXECVE_SKEL) $(PROP_READ_SKEL) $(FILE_ACCESS_SKEL) $(MASSDELETE_DETECT_SKEL) $(EXFIL_DETECT_SKEL) $(ACCESSIBILITY_DETECT_SKEL) $(FILELESS_DETECT_SKEL) $(MEDIAPROJ_ABUSE_SKEL) $(LIBBPF_A)
	mkdir -p $(dir $@)
	$(CC) $(MOD_CFLAGS) -c $< -o $@
...
ALL_BPF_OBJS := $(SYSC_BPF_OBJ) $(FUNC_BPF_OBJ) $(LIB_BPF_OBJ) $(CORR_BPF_OBJ) \
                ...
                $(ACCESSIBILITY_DETECT_BPF_OBJ) $(FILELESS_DETECT_BPF_OBJ) $(MEDIAPROJ_ABUSE_BPF_OBJ)
