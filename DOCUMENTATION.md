...
itself — use standalone `correlate` for that). It owns no BPF object of its
own: it is a thin coordinator (`src/trace/trace.c`) over the engines'
setup/run/teardown phases (§1), driven by a `struct engine engines[3]`
descriptor table (`trace.c:176-183`) so the build/arm/launch/drain/teardown/
merge phases are one loop over the table (`cmd_trace`, `trace.c:186`) instead
of a block per engine.

**`correlate` and `dump` are not composable into `trace`** (a prior revision
...
- **Coordinator mode plumbing:** `struct ares_run_ctx` (`src/common/launch.h`)
  ...
  engine's own argv. The driver symbols (`<engine>_setup`/`_run`/`_teardown`
  for `syscalls`/`funcs`/`lib`, still declared alongside `correlate`/`dump`'s
  for other callers of the header) live in `src/common/engine_driver.h` (AA3)
  and are kept global through the partial-link so `trace.part.o` can call
...
- **No section markers.** Every flag is recognized by its own letter/name and
  ...
  original token appended **verbatim** to that engine's argv, so its own
  `argp` parses exactly what it would have gotten before this rework; a
  `mod:` spec is a hard error (`mod` isn't a trace engine). Shared flags
...
- **CLI:** `ares trace (-P <pkg> | -p <pid[,pid…]>) [-o <prefix>] [-A <act>]
  ...
  merged `<prefix>` combining all three (`jsonl_merge`, `trace.c:341`), all
  ingestable by the unified MCP today. At least one engine must end up
  enabled or `trace` errors.
...
- **Detectability:** loud only if `funcs` ends up enabled — it loads the
  `funcs` uprobe (entry `BRK`) alongside the `syscalls` kprobe, so a run
  needs `funcs` in the mix to leave the stealthy side of the firewall (§9).
