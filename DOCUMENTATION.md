...
- Attaches uprobes/uretprobes to functions selected by **probe specs**
  ...
  values, call→return timing, and a call stack. The legacy `-I`/`-i`/`-r` regex
  flags are deprecated in favor of `-e SPEC`/`-F FILE` (a one-time stderr warning
  prints the equivalent spec line the first time each deprecated flag is used);
  they still work unchanged this release.
...
- **CLI:** `ares dump {-P PACKAGE | PACKAGE | -p PID[,PID...]} PATTERN [-F FILE] [-A ACTIVITY] [-d DIR] [--on-map] [--raw] [-q] [--siblings] [--no-follow-fork]`.
