...
int cmd_prelude_check(void)
{
    ...
    printf("  uprobe only:    ares funcs -p %d -I prelude-check -i target_uprobe\n", pid);
    printf("  uretprobe only: ares funcs -p %d -I prelude-check -r target_uretprobe\n", pid);
    printf("  both:           ares funcs -p %d -I prelude-check -i \"target_\"\n\n", pid);
