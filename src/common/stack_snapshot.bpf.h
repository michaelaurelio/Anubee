 ...
 *
 *   #define ANUBEE_SNAPSHOT_RB events         // syscalls: their ringbuf name
 *   #define ANUBEE_SNAPSHOT_RB events_rb      // funcs: their ringbuf name
 ...
 */
#ifndef ANUBEE_STACK_SNAPSHOT_BPF_H
#define ANUBEE_STACK_SNAPSHOT_BPF_H
...
static __always_inline __u64 anubee_hash_stack(__u64 *stack, int n, __u32 tgid, int cap)
{
 ...
 * value for a stack record (e.g. SYSC_EV_STACK or ANUBEE_EVENT_STACK). */
static __always_inline void anubee_emit_stack_snapshot(struct pt_regs *user_regs,
						     __u32 tgid, __u32 tid,
						     __u64 sid, __u32 ev_type)
{
	struct anubee_stack_snapshot *s =
		bpf_ringbuf_reserve(&ANUBEE_SNAPSHOT_RB, sizeof(*s), 0);
	if (!s) {
		bump_dropped();
		return;
	}
	 ...
	 * ANUBEE_SNAP_CHUNK pieces; on the first faulting (unmapped) chunk, latch
	...
	int stop = 0;
	#pragma clang loop unroll(full)
	for (int i = 0; i < ANUBEE_SNAP_MAX / ANUBEE_SNAP_CHUNK; i++) {
		if (!stop && s->sp &&
		    bpf_probe_read_user(s->snap + i * ANUBEE_SNAP_CHUNK,
					ANUBEE_SNAP_CHUNK,
					(const __u8 *)s->sp + i * ANUBEE_SNAP_CHUNK) == 0)
			s->snap_len = (i + 1) * ANUBEE_SNAP_CHUNK;
		else
			stop = 1;
	}
	...
	s->truncated = (s->snap_len == ANUBEE_SNAP_MAX);
	if (s->truncated)
		cov_bump(COV_TRUNC);
	bpf_ringbuf_submit(s, 0);
}

#endif /* ANUBEE_STACK_SNAPSHOT_BPF_H */
