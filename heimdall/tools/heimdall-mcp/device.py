"""On-device heimdall control for the MCP server.

Drives the heimdall binary on a connected device over adb to (a) list the native
libraries an app loads and (b) dump a (possibly decrypted) library from the app's
live memory, pulling the rebuilt .so back to the host. heimdall is run under
...
Configuration (environment):
  HEIMDALL_ADB           adb executable (default: "adb")
  HEIMDALL_BIN           heimdall path on device (default: /data/local/tmp/heimdall)
  HEIMDALL_SHELL_PREFIX  wrap the device command, e.g. "su -c" on a Magisk device
                         (heimdall needs root; set this if adbd isn't already root)
  HEIMDALL_SERIAL        target a specific device (adb -s <serial>)

The line parsers (`parse_lib_lines` / `parse_dump_lines`) are pure functions over
heimdall's stdout/stderr and are unit-tested without a device.
"""
...
ADB = os.environ.get("HEIMDALL_ADB", "adb")
BIN = os.environ.get("HEIMDALL_BIN", "/data/local/tmp/heimdall")
SHELL_PREFIX = os.environ.get("HEIMDALL_SHELL_PREFIX", "").strip()
SERIAL = os.environ.get("HEIMDALL_SERIAL", "").strip()
...
def parse_lib_lines(text):
    """Parse heimdall `-l` output into a list of per-segment mapping records."""
    out = []
...
def parse_dump_lines(text):
    """Parse heimdall `-D` per-module `[dump]` lines into structured records."""
    out = []
    for line in text.splitlines():
...
def _adb(args, timeout=120):
    ...
    try:
        ...
        raise RuntimeError(f"adb not found ({ADB!r}); set HEIMDALL_ADB")
...
def _run_heimdall(args, seconds):
    """Run heimdall on the device for `seconds` (SIGINT-terminated via timeout),
    as root if HEIMDALL_SHELL_PREFIX is set. Returns combined stdout+stderr."""
    inner = "timeout -s INT %d %s %s" % (
    ...
    try:
        ...
        # timeout should have stopped heimdall device-side; return what we got
...
def list_libraries(package, seconds=8, activity=None):
    """Launch the app via heimdall `-l` for `seconds`, then return the native
...
def dump_library(package, pattern, seconds=12, activity=None, out_dir=None):
    """Run heimdall `-l -D <pattern>` on the device for `seconds`, dumping every
    loaded library whose basename matches `pattern` (glob ok, e.g. 'e_*') from
    live memory on exit, then pull the rebuilt .so(s) to the host."""
    _check_pkg(package)
    ...
    secs = _clamp_seconds(seconds)
    out_dir = os.path.abspath(out_dir or os.path.join(os.getcwd(), "heimdall-dumps"))
    os.makedirs(out_dir, exist_ok=True)

    devdir = "/data/local/tmp/heimdall-mcp-" + uuid.uuid4().hex[:12]
...
def _diagnose(log):
    ...
    if "not found" in low or "no such file" in low:
        return ("heimdall not found on device — push it to %s "
                "(make push) or set HEIMDALL_BIN" % BIN)
    if "run as root" in low or "permission denied" in low or "operation not permitted" in low:
        return ("heimdall needs root — set HEIMDALL_SHELL_PREFIX='su -c' "
                "or run `adb root`")
    if "could not resolve uid" in low:
        return "package not installed or not launchable on this device"
    snippet = "\n".join(log.strip().splitlines()[-6:])
    return "no output matched; tail of heimdall log:\n" + snippet if snippet else "no output from heimdall"
