...
// Subsumes anubee_drops_report (ring/queue drops are coverage fields here).
#ifndef __ANUBEE_COMMON_COVERAGE_H
#define __ANUBEE_COMMON_COVERAGE_H

#include "common/cfi_unwind.h"    // enum cfi_stop_reason
#include "common/coverage_slots.h"

struct anubee_sink;   // fwd; full def in common/emit.h
...
// guards the coupling.
#define ANUBEE_CFI_STOP_N  (CFI_SNAP_CFI_GET_NULL + 1)

struct anubee_coverage {
	...
	unsigned long long cfi_stop[ANUBEE_CFI_STOP_N];
...
static inline int anubee_cfi_stop_is_blind(int reason)
{
	return reason != CFI_OK && reason != CFI_SNAP_PC_ZERO;
}

// libbpf-backed reader, header-only (mirrors anubee_drops_read in runtime.h) so
...
static inline unsigned long long anubee_coverage_read(int map_fd, unsigned int slot)
{
...
#endif /* __ANUBEE_COMMON_COVERAGE_H */
