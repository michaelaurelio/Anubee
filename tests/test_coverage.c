// Host unit tests for anubee_coverage_report (CR5). Pure struct -> output.
...
static void emit_json(const struct anubee_coverage *cov, char *out, size_t n)
{
	char path[] = "/tmp/anubee_cov_XXXXXX";
	int fd = mkstemp(path); assert(fd >= 0); close(fd);
	struct anubee_sink s = {0};
	assert(anubee_sink_open(&s, path, "coverage", /*jsonl=*/1) == 0);
	anubee_coverage_report(&s, cov);
	anubee_sink_close(&s);
...
int main(void)
{
	...
	struct anubee_coverage clean = {0};
	...
	struct anubee_coverage c = {0};
	c.engine = "correlate"; c.decode_partial = 1;
	emit_json(&c, j, sizeof(j));
	CHECK(strstr(j, "\"decode_partial\":true"));

	// 5. Stop-name mapping.
	CHECK(strcmp(anubee_cfi_stop_name(CFI_NO_FDE), "no_fde") == 0);
	CHECK(strcmp(anubee_cfi_stop_name(CFI_RUN_FAIL), "run_fail") == 0);
	CHECK(anubee_cfi_stop_is_blind(CFI_NO_FDE));
	CHECK(!anubee_cfi_stop_is_blind(CFI_SNAP_PC_ZERO));

	// 6. returns_mode off -> no "returns" key even with counts set.
	struct anubee_coverage r0 = {0};
	r0.engine = "correlate";
